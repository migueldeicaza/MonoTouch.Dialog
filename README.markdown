...
### Accessing the Elements API from the Reflection API: IndexTags ###

In some cases, you need to access the actual elements that were generated by the
BindingContext. For example, you might want to change an [Entry] element's value
in reaction to an [OnTap] event. 

Example: you display the following class with the Reflection API.
...
Note the [IndexTags] attribute that is applied to FirstValue, SecondValue and Result.
In your callbacks class (the first argument to the BindingContext constructor), 
you create the callbacks Add and Multiply:

	public void Add() {
		// context is the BindingContext instance.
		// you need to make sure the callbacks object can access the BindingContext.
		
		float result = 0;
		foreach (Element el in context.GetElementsForTag("value")) {
			var eel = el as EntryElement;
			if (eel != null) {
				// TODO: parse value in eel and add it to result
			}
		}
		EntryElement resultEl = context.GetElementsForTag("result").FirstOrDefault() as EntryElement;
		if (resultEl != null)
...
For a fully functional example, see the sample app under Reflection API -> Index Tags: Animals.  
