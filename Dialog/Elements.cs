...
{
	/// <summary>
	/// Base class for all elements in Dialog
	/// </summary>
	...
	public partial class StyledMultilineElement : StyledStringElement, IElementSizing {
		...
		public virtual nfloat GetHeight (UITableView tableView, NSIndexPath indexPath)
		{
			...
			nfloat height = c.StringSize ( captionFont, maxSize, LineBreakMode).Height;
			...
			if (!String.IsNullOrEmpty (v)) {
				...
				if (this.style == UITableViewCellStyle.Subtitle) {
					...
					nfloat vheight = v.StringSize ( subtitleFont, maxSize, LineBreakMode).Height;
					if (vheight > height)
						height = vheight;
				}
			}
	...
	public partial class EntryElement : Element {
		...
		//
		CGPoint ComputeEntryPosition (UITableView tv, UITableViewCell cell)
		{
			...
			foreach (Section s in sections) {
				...
				foreach (var e in s.Elements) {
					...
					if (ee != null
						...
						maxWidth = (nfloat)Math.Max ((nfloat)size.Width, (nfloat)maxWidth);
						maxHeight = (nfloat)Math.Max ((nfloat)size.Height, (nfloat)maxHeight);
					}
				}
			}

			return new CGPoint((nfloat)25 + (nfloat)Math.Min((nfloat)maxWidth, 160), maxHeight);
		}
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			CGPoint size = ComputeEntryPosition (tv, cell);
			nfloat yOffset = (cell.ContentView.Bounds.Height - size.Y) / 2 - 1;
			nfloat width = cell.ContentView.Bounds.Width - size.X;
			if (textalignment == UITextAlignment.Right) {
				// Add padding if right aligned
				width -= 10;
			}
			var entryFrame = new CGRect (size.X, yOffset, width, size.Y);
...
//		
//		protected NSDate GetDateWithKind (NSDate dt)
//		{
...
//		}
		...
		static CGRect PickerFrameWithSize (CGPoint size)
		{                                                                                                                                    
			...
			case UIInterfaceOrientation.LandscapeRight:
				fX = (screenRect.Height - size.X) /2;
				fY = (screenRect.Width - size.Y) / 2 -17;
				break;
				
			case UIInterfaceOrientation.Portrait:
			case UIInterfaceOrientation.PortraitUpsideDown:
				fX = (screenRect.Width - size.X) / 2;
				fY = (screenRect.Height - size.Y) / 2 - 25;
				break;
			}
			
			return new CGRect (fX, fY, size.X, size.Y);
		}                                                                                                                                    
...
//			datePicker.Frame = PickerFrameWithSize (datePicker.SizeThatFits (CGPoint.Empty));
		}
	}
...
//		}
//		
////		public override string FormatDate (NSDate dt)
////		{
...
//	}
	...
	/// <summary>
	/// Sections contain individual Element instances that are rendered by Dialog
	/// </summary>
	/// <remarks>
	...
	/// </summary>
	/// <remarks>
	///    At least one RootElement is required to start the Dialogs
	///    process.   
	/// 
