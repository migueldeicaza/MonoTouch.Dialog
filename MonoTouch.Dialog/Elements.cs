...
namespace MonoTouch.Dialog
{
	...
	public class BooleanElement : BoolElement
	{
		...
		{
			if (sw == null) {
				sw = new UISwitch (new RectangleF (198, 12, 94, 27)) { BackgroundColor = UIColor.Clear, Tag = 1, On = Value };
				sw.ValueChanged += delegate { Value = sw.On; };
			}
	...
	public class FloatElement : Element
	{
		...
		{
			if (slider == null) {
				slider = new UISlider (new RectangleF (10f, 12f, 280f, 7f)) { BackgroundColor = UIColor.Clear, MinValue = this.MinValue, MaxValue = this.MaxValue, Continuous = true, Value = this.Value, Tag = 1 };
	...
	public class EntryElement : Element
	{
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			if (entry == null) {
				...
				entry = new UITextField (new RectangleF (size.Width, (cell.ContentView.Bounds.Height - size.Height) / 2 - 1, 320 - size.Width, size.Height)) { Tag = 1, Placeholder = placeholder ?? "", SecureTextEntry = isPassword };
				entry.Text = Value ?? "";
				entry.AutoresizingMask = UIViewAutoresizing.FlexibleWidth | UIViewAutoresizing.FlexibleLeftMargin;
				
				entry.ValueChanged += delegate { Value = entry.Text; };
				entry.Ended += delegate { Value = entry.Text; };
				entry.ShouldReturn += delegate {
					EntryElement focus = null;
					foreach (var e in (Parent as Section).Elements) {
						if (e == this)
							focus = this; else if (focus != null && e is EntryElement)
	...
	public class DateTimeElement : StringElement
	{
		...
		{
			var picker = new UIDatePicker (RectangleF.Empty) { AutoresizingMask = UIViewAutoresizing.None, Mode = UIDatePickerMode.Date, Date = DateValue };
			return picker;
		}
