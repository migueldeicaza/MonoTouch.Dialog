...
namespace MonoTouch.Dialog
{
	...
	public class BindingContext : IDisposable
	{
		...
		{
			if (mi is FieldInfo)
				return ((FieldInfo)mi).FieldType; else if (mi is PropertyInfo)
			...
			foreach (var mi in members) {
				...
				foreach (var attr in attrs) {
					...
					if (attr is CaptionAttribute)
						caption = ((CaptionAttribute)attr).Caption; else if (attr is SectionAttribute) {
						if (section != null)
							root.Add (section);
				...
				if (mType == typeof(string)) {
					...
					foreach (object attr in attrs) {
						if (attr is PasswordAttribute)
							pa = attr as PasswordAttribute; else if (attr is EntryAttribute)
							ea = attr as EntryAttribute; else if (attr is MultilineAttribute)
							multi = true; else if (attr is HtmlAttribute)
							html = attr;
					...
					if (pa != null)
						element = new EntryElement (caption, pa.Placeholder, value, true); else if (ea != null)
						element = new EntryElement (caption, ea.Placeholder, value); else if (multi)
						element = new MultilineElement (caption, value); else if (html != null)
					...
					foreach (object attr in attrs) {
						if (attr is DateAttribute)
							asDate = true; else if (attr is TimeAttribute)
							asTime = true;
					}
					
					if (asDate)
						element = new DateElement (caption, dateTime); else if (asTime)
		...
		public virtual void Fetch ()
		{
			foreach (var dk in mappings) {
				...
				if (element is DateTimeElement)
					SetValue (mi, obj, ((DateTimeElement)element).DateValue); else if (element is FloatElement)
					SetValue (mi, obj, ((FloatElement)element).Value); else if (element is BooleanElement)
					SetValue (mi, obj, ((BooleanElement)element).Value); else if (element is CheckboxElement)
					SetValue (mi, obj, ((CheckboxElement)element).Value); else if (element is EntryElement)
					SetValue (mi, obj, ((EntryElement)element).Value); else if (element is ImageElement)
					SetValue (mi, obj, ((ImageElement)element).Value); else if (element is RootElement) {
